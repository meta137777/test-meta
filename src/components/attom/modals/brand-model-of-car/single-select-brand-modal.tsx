...
export default function SingleSelectBrandModel({
  models,
  singleSelection,
  mainValue,
  customHandleChange,
  ...
  const handleBrandClick = (_id, selecteBrandObj) => {
  ...
  const handleModelClick = (_id, selectedModelObj, brand) => {
  ...
  if (inputText) {
    models?.filter((el) => {
      if (el.alias.includes(inputText)) {
        filteredModel.push({
          brand: el.alias,
        });
