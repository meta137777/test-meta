...
import { modalSaveSearchInitValue } from "./form/initial-values";
import { ModalSaveSearchTyeProps } from "./type";
...
export default function ModalSaveSearch({
  ...
  defaultValues,
  models,
}: ModalSaveSearchTyeProps) {
  // @@@___________________ User info selector ___________________@@@
  const { userInfo } = useAppSelector(authSelector);
  // @@@___________________ Edit save searches selector ___________________@@@
  const { editSaveSearch } = useAppSelector((state) => state.brandModel);

  // @@@___________________ Disapcth ___________________@@@
  ...
  // @@@___________________ Close modal handler ___________________@@@
  const handleClose = () => setOpen(false);

  // @@@___________________ Formik submit ___________________@@@
  const onSubmit = (values: any) => {
    if (setDefaultvalue) {
      setDefaultvalue({ ...defaultValues, loading: true });
    }
    ...
    PostFilterSave(postData)
      .then(() => {
        if (setDefaultvalue) {
          setDefaultvalue({ error: false, loading: false, data: postData });
        }
      ...
      .catch(() => {
        if (setDefaultvalue) {
          setDefaultvalue({ error: true, loading: false, data: [] });
        }
  ...
  const formik = useFormik({
    initialValues: modalSaveSearchInitValue(defaultValues),
    onSubmit,
  });

  // @@@___________________ max price for select price ___________________@@@
  const maxPrice = staticData.prices.filter((p) => {
    return p.value > formik?.values?.min_price;
  });

  // @@@___________________ Brand Model handle change ___________________@@@
