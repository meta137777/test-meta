  ...
  return (
    <>
      ...
      <div className="sticky top-0 z-[100] bg-[#F3F8FF]">
        <section className="max-w-[1336px] mx-auto px-2 py-4">
          <nav className="h-fit">
            <div className="grid grid-cols-7 gap-4 items-center justify-between">
              <span className="lg:col-span-2 flex justify-start md:col-span-1 col-span-3 ">
                <Link href={"/"} className="text-right">
                  <img
                    className="md:w-12 w-16 cursor-pointer lg:mx-0 mx-0"
              ...
              <div className="flex justify-end items-center mr-auto lg:col-span-2 md:col-span-3 col-span-4 gap-4">
                <Link
                  href="/car-sale"
                  className="whitespace-nowrap bg-white text-blue border border-blue-200 px-6 py-2 rounded text-xs font-bold md:block hidden"
                >
                  ثبت آگهی
                </Link>
                ...
                {!isTokenExpired(win?.localStorage.getItem("userToken")) ? (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <Link
                      href={"/auth/check"}
                      className="whitespace-nowrap text-blue border border-blue-100 bg-blue-100 px-6 py-2 rounded text-xs font-bold "
                    >
                      ورود | ثبت‌نام
                    </Link>
                  </div>
      ...
      {/* Bottom Navigation */}
      <div
        className="tablet:hidden flex bg-white fixed bottom-0 md:pt-2 sm:pt-3 pt-2 md:px-6 sm:px-3 px-4 w-full justify-between  rounded-tr-3xl rounded-tl-3xl"
        style={{ boxShadow: "0px -1px 4px rgba(0, 0, 0, 0.04)", zIndex: 200 }}
      >
      ...
      {showSubMenu && (
        <div
          className="fixed bg-white w-full h-full pt-16 bottom-[-6rem] top-0"
          style={{
            zIndex: 102,
          }}
        >
...
const SearchInput = () => {
  ...
  const searchHandler = async (value: any) => {
    if (value !== "" && value?.length >= 3) {
      const tempSearch = await GetAdSaleSearchKeywords(value);
