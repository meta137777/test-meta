import { postExhibitorUpdateInfoAPI } from "@/apis/authentication@";
...
const PersonalInfoForm = ({ handleNext, setShowStepper }: any) => {
  const userInfo =
    checkExistWindow() && JSON.parse(localStorage.getItem("userInfo"));
  let phone_number =
    checkExistWindow() && JSON.parse(localStorage.getItem("phone_number"));
  ...
  const onSubmit = (values: any) => {
    ...
    postExhibitorUpdateInfoAPI({ body: postData, phone: phone_number })
      .then((res) => {
        handleNext();
        // setStatus({});
        setShowStepper(true);
