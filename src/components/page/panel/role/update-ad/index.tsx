...
import MyAdMultipleImageModal from "@/attom/cards/panel@/card-my-ad/components/my-ad-mutiple-image";
    ...
    <div className="custom-shadow md:p-5 p-3 rounded-md mb-10">
      ...
      <div className="mt-3">
        <form onSubmit={formik.handleSubmit} className="flex flex-col">
          ...
          <MyAdMultipleImageModal advertiser_id={advertise.advertiser_id} setLoadImage={setLoadImage} image_guides={advertise.image_guids} tempStatus="update" tempLastImagesLength={advertise.image_guids.length}/>

          {/* <ImageUploader
            imgsSrc={[...new Map(tempImages.map(v => [v.id, v])).values()].map((item: any): ImageFileType => {
              return {
                id: item.id,
            ...
            setImgsSrc={setTempImages}
            title="تصاویر مورد نظر خود را بارگذاری کنید."
            imageCount={20 - [...new Map(tempImages.map(v => [v.id, v])).values()].length - 1}
          /> */}
