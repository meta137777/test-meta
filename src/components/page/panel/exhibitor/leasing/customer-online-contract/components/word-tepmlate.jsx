...
function WordTemplate() {
  ...
  const checks_list = [
    state?.installment_info?.checks?.map((check, index) => ({
      ...
      amount_in_number: NumberSeprator(check?.amount_in_number),
  ...
  const data0 = {
    ...
    vehicle_info_vehicle_amount_in_number:
     NumberSeprator(state?.vehicle_info?.vehicle_amount_in_number),
    ...
    NumberSeprator(state?.installment_info?.total_amount_in_number),
    ...
    NumberSeprator(state?.installment_info?.prepaid_amount_in_number),
    ...
    NumberSeprator(state?.installment_info?.refund_amount_in_number),
    ...
    NumberSeprator(state?.installment_info?.check_templates[0]?.amount_in_number),
    ...
    NumberSeprator(state?.installment_info?.check_templates[
        state?.installment_info.check_templates?.length - 1
      ].amount_in_number),
    ...
    installment_info_amount : NumberSeprator(state?.vehicle_info?.vehicle_amount_in_number - state?.installment_info?.prepaid_amount_in_number),
  ...
  const data1 = {
    ...
    // Guarantors
    // First Guarantor
    applicant_info_guarantors_0_name:
      state?.applicant_info?.guarantors[0]?.name,
  ...
  const docNumber = () => {
    let docInfo;
    if (facility_amount <= 3000000000) {
      // مبلغ تسهیلات بین ۰ تا ۳۰۰
      docInfo = {
        data: data0,
        docNum: wordTemplate0,
      };
    } else if (facility_amount > 3000000000 && facility_amount <= 6000000000) {
  ...
  const generateDocument = (fileFormat) => {
    ...
    loadFile(docNumber().docNum, function (error, content) {
      ...
      if (fileFormat === "word") {
        ...
        postConvertDoxcToPdf(wordFormData)
          .then((response) => {
