...
const CustomerCarInfoForm = () => {
  ...
  const initialValues = {
    ...
    brand: state?.vehicle_info?.brand,
  ...
  const validationSchema = Yup.object({
    ...
    engin_number: Yup.string().required(
  ...
  useEffect(() => {
    ...
    formik.setFieldValue("model", state?.vehicle_info?.model);
    formik.setFieldValue("type", state?.vehicle_info?.type);
    formik.setFieldValue("brand", state?.vehicle_info?.brand);
  }, [state?.vehicle_info]);
    ...
    <form onSubmit={formik.handleSubmit}>
      <div className="flex flex-col">
        <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6 mt-4">
          <DynamicBrandModal
            defaultValue={
              !state?.vehicle_info?.type == "undefined" ||
        ...
        <div className="flex flex-col gap-4 my-8">
          ...
          <p className="text-sm">
            ...
            <samll className="font-light text-xs">ریال</samll>) ، میزان تسهیلات
