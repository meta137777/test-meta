...
export default function CustomerUploadCheck({ reason }) {
  ...
  const guarantors_checks =
    state?.installment_info?.guarantors_check_templates?.map((check, index) => {
      ...
      return {
        ...
        order_back: (guarantors_checks_order + order + 2).toString(),
        check_no: "",
        date_in_number: check.date_in_number,
        date_in_letters: convertToPersianDate(check.date_in_number),
        amount_in_number: check.amount_in_number,
  ...
  console.log("guarantors_checks", guarantors_checks);
  console.log("guarantors_checks_order", guarantors_checks_order);
      ...
      <form onSubmit={formik.handleSubmit} className="flex flex-col">
        ...
        {guarantors_checks?.map((checkTemplate, index) => (
          <UploadCheck
            ...
            check_national_id={
              state?.installment_info?.guarantors_check_templates[index]?.check_national_id
            }
            pay_to={state?.installment_info?.guarantors_check_templates[index]?.pay_to}
          />
        ))}
