...
import ExhibitorBranches from "./components/exhibitor-branches";
...
export default function ContinueExhibitionRegister() {
  ...
  const [branchAmount, setBranchAmount] = useState(1);
  ...
  const onSubmit = (values) => {
    ...
    // postData.append("exhibition_branches_infos",)
    ...
    <div className="border border-gray-150 p-4 rounded-lg">
      ...
      <form onSubmit={formik.handleSubmit}>
        <div className="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-4 mt-6">
          ...
          <FormInput
            formik={formik}
            classNames="md:col-span-4"
            name="exhibition_about_us"
            label="توضیحات درباره نمایشگاه"
            placeholder="توضیحات"
          />
          <FormInput
            formik={formik}
            classNames="md:col-span-2 "
            name="exhibition_working_hours"
            label="ساعت کاری نمایشگاه"
            placeholder="شنبه تا چهارشنبه، 9 تا 18"
          />
          <FormInput
            formik={formik}
            classNames="md:col-span-2 "
            name="exhibition_local_phone_number"
            label="شماره تماس نمایشگاه"
            placeholder="02188108260 و 02188108261 "
          />
        </div>

        <h1 className="font-bold text-xl mt-5 mb-4 text-blue border-b border-b-gray-200 pb-4 w-full">
          اطلاعات شعب
        </h1>
        {Array.from(Array(branchAmount)).map((item, index) => {
          return (
            <div key={index}>
              <ExhibitorBranches number={index + 1} formik={formik} />
            </div>
          );
        })}

        <IoMdAddCircle
          className="cursor-pointer mx-auto mt-5"
          color="blue"
          size="50px"
