import { CustomInput } from "@/attom/form@/components@/inputs/custom-input";
...
const StepOne = () => {
  ...
  const [mileAgeValue, setMileAgeValue] = useState({
    formattedValue: mileage?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","),
    value: mileage,
  });
  ...
  const mileageChangeHandler = (e) => {
    ...
    if (!isNaN(sanitizedValue)) {
      ...
      dispatch(ADD_MILEAGE(+sanitizedValue));
    } else {
      return;
    }
  };
    ...
    <div className="rounded-xl p-4 bg-white tablet:w-[27rem] md:w-[24rem] w-full h-full flex flex-col tablet:flex-col gap-8 mx-auto">
      ...
      <div>
        <CustomInput
          ...
          label="کارکرد"
          name="mileAge"
          value={mileAgeValue.formattedValue}
          placeholder="280000"
