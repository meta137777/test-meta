...
export const filter = createSlice({
  ...
  reducers: {
    ...
    SET_SHOW_NULL_BTN: (state, { payload }) => {
    ...
    ADD_SORT: (state, { payload }) => {
      ...
      state.ascending = payload.ascending;
    },
    SET_SHOW_CAR: (state, { payload }) => {
      if (
        !state.shownCars.some(
          (cars) =>
            cars.brand === payload.brand &&
            cars.is_car_made_aboard === payload.is_car_made_aboard &&
            cars.model === payload.model &&
            cars.type === payload.type
        )
      ) {
